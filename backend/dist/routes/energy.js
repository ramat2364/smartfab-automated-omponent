...
router.get('/overview', auth_1.authenticateToken, auth_1.checkPlantAccess, async (req, res) => {
    try {
        ...
        const energyReadings = await db_1.default.energyReading.findMany({
            where: whereClause,
            include: {
        ...
        const productionEntries = await db_1.default.productionEntry.findMany({
            where: {
...
router.get('/machines', auth_1.authenticateToken, auth_1.checkPlantAccess, async (req, res) => {
    try {
        ...
        const readings = await db_1.default.energyReading.findMany({
            where: whereClause,
            include: {
                machine: {
        ...
        const machinesBreakdown = Object.values(machineMap).map((m) => ({
            ...m,
...
router.get('/pat', auth_1.authenticateToken, auth_1.checkPlantAccess, async (req, res) => {
    try {
        const { plantId } = req.query;
        const queryPlantId = plantId || req.user?.plantAccessId;
        ...
        for (const plant of plants) {
            const energySum = await db_1.default.energyReading.aggregate({
                where: {
                    plantId: plant.id,
            ...
            const prodSum = await db_1.default.productionEntry.aggregate({
                where: {
                    plantId: plant.id,
            ...
            let targetSec = 0.80;
            if (plant.name.includes('Pune'))
                targetSec = 0.90;
            if (plant.name.includes('Nashik'))
                targetSec = 0.85;
